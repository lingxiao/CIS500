...
------------------------------------------------------------------------------------------
-- | 
-- | Module : PlayBasics: Functional Programming in Coq
-- | Creator: Xiao Ling
-- | Date   : 1/13/2016
...
Eval simpl in (next_weekday (next_weekday saturday)).
   (* ==> tuesday : day *)
Eval simpl in (prev_weekday (next_weekday saturday)).
   (* ==> firday  : day *)
...
Example test_circular:
  (next_weekday (prev_weekday tuesday)) = tuesday.

Proof. simpl. reflexivity. Qed.

(* a wrong example

Example test_circular_false:
  (next_weekday (prev_weekday tuesday)) = monday.
...
(*
  note the sugar and module syntax
*)
Check O.
Check (S O).
Check 1.
Check Playground1.O.
Check (Playground1.S Playground1.O).
...
(** These are all things that can be applied to a number to yield a
    ...
    so functions have reduction rules associated with it,
    but type constructors cannot be reduced
*)
...
(* Note though [oddb] calls a fixpoint function, it is not itslef*)
...
Proof. reflexivity.  Qed.
Example test_oodb3:    (oddb 13) = true.
Proof. reflexivity.  Qed.
...
(* nat defined in prelude *)
Eval simpl in (plus 12 10).
...
(** The form of this theorem and proof are almost exactly the
    ...
    ie [simpl] reduces the expression until end and check equality
...
(*
   Solution:  = forall n : nat, n + 0 = n : Prop
*)
...
(*
   Solution:  = forall n : nat, n = n : Prop
*)

(** Explain the difference.  [] *)
(*
   Solution: in [0 + n = n] or [add 0 n = n], [+] pattern match on first
             argument, which is [0+n].
*)
...
(** Aside from unit tests, which apply functions to particular
    ...
    So how do we read [intros]? Suppose n? for some n? how does it generalize
    if we are allowed to pick some arbitrary n \in Nat?
*)
...
Proof.
  ...
  rewrite -> H. (* Rewrite the goal using the hypothesis *)
                (* read left to right in H, replace n with m *)
  reflexivity.  Qed.
...
(*
  why does [rewrite -> plus_1_l]
  replace the [(1+n)] in [(1+n)*m = m + n * m] only?
*)
...
Proof.
  intros n.
  destruct n as [| n'].   (* [O] has no subcomponent, on the other hand we have [S n']*)
    - reflexivity.          (* assert: [0 + 1    = 0 = false]*)
    - reflexivity.          (* assert: [S n' + 1 = 0 = false]*)
Qed.
...
Proof.
  intros b c.
  destruct b.  
  - destruct c.     (* focus on subgoal 1: true && c == c && true *)
    + reflexivity.  (* true && true   = true && true  *)
    + reflexivity.  (* true && false  = true && false *)
  - destruct c.     (* false && c     = false && c   *)
    + reflexivity.
    + reflexivity.
    (*Defined.
    Print andb_commutative.*)
Qed.
...
(*
  todo: what does the 2nd reflexivity do?
*)
...
(*
  todo : walk through this with Professor, what exactly
         is each function doing?
*)
Theorem andb3_exchange :
  forall b c d, (b && c) && d = (b && d) && c.
Proof.
  intros b c d. destruct b.   (* b && c && d = b && d && c  for b = true and b = false*)
  - destruct c.               (* let c = true *)
    { destruct d.             (* let d = true *)
      - reflexivity.          
      - reflexivity. }
    { destruct d.             (* let d = false *)
      - reflexivity.
      - reflexivity. }
  - destruct c.               (* let c = true *)
    { destruct d.
...
Theorem orb_commutative: forall b c, b || c = c || b.
Proof.
  intros [] [].
  - reflexivity.
  - reflexivity.
...
Proof.
  intros b c H.    
  destruct b.      (* let b = true so we have [true && c = true] *)
  - destruct c.    (* let c = true and c = false *)
    + reflexivity. (* true && true = true *)
    + inversion H. (* but by hypothesis H, b && c = true *)
  - inversion H.   (* but by hypothesis H, b && c = true so
                      by definition of [andb] b cannot be false *)
Qed.    
...
(** **** My Exercises and Questison ***** **)

(*
  Observe the hiearchy of universe here
*)
Check true. (* : bool *)
Check 12.   (* : nat  *)
Check nat.  (* : Set  *)
...
(** Use the tactics you have learned so far to prove the following 
   ...
   read:
   Pi_{f : Bool -> Bool} (Pi_{x : Bool} f x = x)
   =>
   Pi_{b : Bool} f (f b) = b
...
(*
  alternate profs using more strats. To extract proof:

  Defined.
  Print identity_fn_applied_twice_2.

*)
Theorem identity_fn_applied_twice_2 : 
  forall  (f : bool -> bool),
  (forall (x : bool), f x = x) ->
  forall  (b : bool), f (f b) = b.
Proof.
...
(** Now state and prove a theorem [negation_fn_applied_twice] similar
    ...
    function no [f] has the property that [f x = negb x].*)
...
Proof.
  ...
  rewrite -> H.
  rewrite -> H.
  rewrite -> negb_involutive.   (* use previous lemma *)
  reflexivity.
Proof.
...
(*
  ...
  so we have:	
  b /\ c = b \/ c  => (b = true & c = true) || (b = false & c = false)
...
Proof.
  intros b c H.
  destruct b.
    + destruct c.    (* let b = true *)
      - reflexivity. (* let c = true, then H satisified and we have b = c *)
      - inversion H. (* let c = false, but by H this cannot be *)
    + destruct c.    (* let b = false *) 
      - inversion H. (* let c = true, but by H this cannot be *)
      - reflexivity. (* let c = false, then H is satisfied and we have b = c *)
Qed.
...
(* (b) prove bin_un (b +> 1) = 1 + (bin_un b) *)
Theorem pathIdentity:
  forall (b : bin), bin_un (incr_bin b) = 1 + bin_un b.
Proof.
  intros b.
  induction b.
  reflexivity.  (* bin_un (BO      +> 1) = 1 + (bin_un BO     )  *)
  reflexivity.  (* bin_un (Even b' +> 1) = 1 + (bin_un (Even b') *)
                (* bin_un (odd  b' +> 1) = 1 + (bin_un (Odd  b') *)
  simpl.        
  rewrite -> IHb.
  reflexivity.  
Qed.
...
Fixpoint bin_un2 (n: bin) : nat := match n with
  | BO       => O
  | Even n'  => bin_un2 n' + bin_un2 n'
  | Odd n'   => S (bin_un2 n' + bin_un2 n')
  end.


Theorem pathIdentity2:
  forall (b : bin), bin_un2 (incr_bin b) = 1 + bin_un2 b.
Proof.
  intros b.
...
(* some ad hoc tests *)
Compute bin_un ((BO +> 100)).
Compute bin_un ((BO +> 13)).
Compute bin_un ((BO +> 0 +> 12 +> 13)).
