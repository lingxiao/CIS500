...
(** This definition still makes perfect sense in the case of
    always-terminating programs, so let's apply it to prove some
    non-deterministic programs equivalent or non-equivalent. *)
(**  Why only "in the case of always-terminating programs"?  Is
    this a good place to add Phil's note that the big-step semantics
    cannot distinguish always-terminating from sometimes-terminating
    programs and the requested pointer to the discussion in
    Smallstep.v? 
 *)
...
(** Another useful fact... *)
(** **** Exercise: 1 star, optional (stequiv_t_update)  *)
Lemma stequiv_t_update : forall (st1 st2 : state),
  st1 ~~ st2 ->
  forall (X:id) (n:nat),
  t_update st1 X n ~~ t_update st2 X n.
Proof.
  (* FILL IN HERE *) Admitted.
(** [] *)
...
Proof.
    ...
    - (* -> *)
      ...
      constructor. { reflexivity. }
      assert (t_update st'0 X (st'0 X) ~~ st'0) as H.
