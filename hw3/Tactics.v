...
(** This chapter introduces several more proof strategies and
    tactics that, together, allow us to prove more interesting
    theorems about the functional programs we have been writing.  We
    will see:
...
(** At this point, we could finish with "[rewrite ->
    eq2. reflexivity.]" as we have done several times before. But we
...
Proof.
  intros l l2' H.
  rewrite H.
  symmetry. apply rev_involutive. (*rewrite rev_involutive.*)
Qed.
...
(** Briefly explain the difference between the tactics [apply] and
    ...
    suppose goal is the same form as hypothesis.
...
(*
  todo: what am I suppose to see here ??
*)
...
(** Coq provides a tactic called [inversion] that allows us to
    ...
    show that the [S] constructor is injective: *)
...
(** It is not clear how we can use the tactics we've seen to
    solve this goal. By writing [inversion H], we ask Coq to generate
    all equations that it can infer from [H] as additional hypotheses,
    replacing variables in the goal as it goes. In the present
    example, this amounts to adding a new hypothesis [H1 : n = m] and
    replacing [n] by [m] in the goal. *)

  inversion H. reflexivity.  Qed.

(** Note that injectivity works for arbitrary constructors, not
    just [S]. Here's a more interesting example that shows how
    multiple equations can be derived at once. *)
...
(** While the injectivity of constructors allows us to reason
    ...
    about constructors and functions, which we will often find
    useful: *)
...
(** However, the second one doesn't look so simple: assuming
    [beq_nat 0 (S n') = true], we must show [S n' = 0], which is
...
(** If we now use [inversion] on this hypothesis, Coq notices
...
(** This is an instance of a more general logical principle
    ...
    contradiction entails anything, even false things.  For
    instance: *)
...
(** If you find the principle of explosion confusing, remember
    ...
    the statement holds. Instead, they are just arguing that the
    situation described by the premise can never arise, so the
    implication is vacuous.  We'll explore the principle of explosion
...
(** Here we are stuck.  The induction hypothesis, [IHn'], does not
    give us [n' = m'] -- there is an extra [S] in the way -- so the
...
(** Notice that both the goal and the induction hypothesis have
    changed: the goal asks us to prove something more general (i.e., to
    prove the statement for _every_ [m]), but the IH is correspondingly
    more flexible, allowing us to choose any [m] we like when we apply
    the IH. *)

    intros m eq.

(** Now we choose a particular [m] and introduce the assumption that
    [double n = double m].  Since we are doing a case analysis on [n],
    we need a case analysis on [m] to keep the two "in sync." *)
...
(** At this point, since we are in the second branch of the [destruct
    ...
    are also in the [S] branch of the induction, this is perfect: if
    we instantiate the generic [m] in the IH with the [m'] that we are
    talking about right now (this instantiation is performed
    automatically by [apply]), then [IHn'] gives us exactly what we
...
(** ... we get stuck: [simpl] doesn't simplify anything at this point,
    ...
    The solution is to manually [unfold] the definition of [square]: *)
...
(** Now we have plenty to work with: both sides of the equality are
    ...
    it is associative, and using this it is not hard to finish the
    proof. *)
...
(** After unfolding [sillyfun] in the above proof, we find that
    ...
    either [n] is equal to [3] or it isn't, so we can use
    [destruct (beq_nat n 3)] to let us reason about the two cases.
...
Theorem combine_split : forall X Y (l : list (X * Y)) l1 l2,
  ...
  combine l1 l2 = l.
Proof.
  intros X Y l l1 l2. induction l.
...
(** We've now seen many of Coq's most fundamental tactics.  We'll
    introduce a few more in the coming lectures, and later in the
    course we'll see some more powerful _automation_ tactics that make
    Coq help us with low-level details.  But basically we've got what
