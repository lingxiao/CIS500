...
(** Suppose we have types [S], [T], [U], and [V] with [S <: T]
    ...
    - [T->T->U <: S->S->V]
      ...
      T -> ( T -> U )  <: S -> ( S -> V )
          ...
          - since S < : T first part is ok, now we require
    ...
    todo: consider these, will be mult choice on final exam
          right or left associativity when looking at types?
...
Where does the type [Top->Top->Student] fit into this order?
   ...
   so we have [Top -> (Top -> Student)], we need to know if

     [Student <: Top -> Student]   or [Top -> Student <: Student]
